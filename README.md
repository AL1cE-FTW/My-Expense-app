...
- **年間表示** — 「月別」「年間」タブで切り替え、収支・予算・Need/Want/Save・カテゴリ別支出・記録一覧を年単位で確認可能(予算目標は月間予算×12で計算)
- **月別の収支推移グラフ** — 年間表示のときだけ、その年の1月〜12月の収入・支出を棒グラフで表示し、月ごとの増減が一目でわかる(バーにカーソルを合わせると金額を表示。縦軸にはキリの良い間隔で金額目盛りを表示)
- **サイドバーナビゲーション(PC)** — 画面幅が広いPCではダークカラーのサイドバーを表示し、「概要」「予算」「Need/Want/Save」「カテゴリ別支出」「記録一覧」の各セクションへワンクリックで移動可能。現在スクロールして見ているセクションのリンクが自動でハイライトされる。スマホなど画面が狭い場合はサイドバーを自動的に非表示にし、従来通り上部ヘッダーのみのレイアウトになる
...
「今月の予算」「カテゴリ別支出」の各カテゴリ名の左には、Need(青)/Want(黄)を示す
