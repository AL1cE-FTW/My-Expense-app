...
- **給与明細の内訳入力** — 収入・カテゴリ「給与」を選ぶと「給与明細の内訳を入力する」ボタンが出現。支給(本給・通勤手当・時間外勤務手当)と控除(健康保険・介護保険・厚生年金・雇用保険料・所得税・住民税・その他控除)を入力すると手取り額(差引支給額)を自動計算し、記録される金額に反映(税金や社会保険料は実際には手元に入らないお金なので、家計簿上の収入・収支には手取り額だけが影響する)。内訳を入力した記録は一覧に「内訳」ボタンが表示され、いつでも支給・控除の内訳を見返せる
...
会社からの給与は、額面(支給合計)と実際に振り込まれる金額(手取り)が
社会保険料・税金などの天引きにより異なります。一般的な給与明細のフォーマットに
合わせて、以下の内訳を記録できます:

1. 種別「収入」・カテゴリ「給与」を選ぶ
2. 「給与明細の内訳を入力する」ボタンを押す
3. **支給**: 本給・通勤手当・時間外勤務手当
   **控除**: 健康保険・介護保険・厚生年金・雇用保険料・所得税・住民税・
   その他控除(慶弔掛金・福祉会費・寮社宅関係・子ども支援金など、会社独自の
   小額の控除項目をまとめて入力)
   を入力すると、手取り額(支給合計 − 控除合計)が自動計算され、
   上の「金額」欄に反映されます
