...
- **サイドバーナビゲーション(PC)** — 画面幅が広いPCではダークカラーのサイドバーを表示し、「概要」「予算」「Need/Want/Save」「記録一覧」の各セクションへワンクリックで移動可能。現在スクロールして見ているセクションのリンクが自動でハイライトされる。スマホなど画面が狭い場合はサイドバーを自動的に非表示にし、従来通り上部ヘッダーのみのレイアウトになる
